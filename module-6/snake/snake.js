...
class TSnakeHead extends TSnakePart {
 ...
 setDirection(aDirection) {
  ...
  update(){
    ...
    if(boardCellInfo.infoType === EBoardCellInfoType.Bait || boardCellInfo.infoType === EBoardCellInfoType.badBait) {
      baitIsEaten(boardCellInfo.infoType);
    }else{
...
export class TSnake {
  ...
  update(){
    ...
    if(this.#head.update()) {
      ...
      if (this.#clonePart === null) { // if there is not something to clone, tail stays normal. 
      this.#tail.update();  
      } else { 
        this.#body.push(this.#clonePart); // Add the cloned part to the body, if there has been a bait eaten 
        this.#clonePart = null; // Reset the clone part, so it goes back to null and not keep cloning the part each time 

      }
    }else if(!this.#isDead){
      this.#isDead = true; 
      GameProps.sounds.deadSound(); // Play dead sound when snake dies from sounds.js
      return false; // Collision detected, do not continue
    }
    return true; // No collision, continue
  ...
  grow () { // this function is called when the snake eats an apple, creating a clone of the last body part. 
    this.#clonePart = this.#body[this.#body.length - 1].clone(); // Clone the last body part (-1 since its a list)
