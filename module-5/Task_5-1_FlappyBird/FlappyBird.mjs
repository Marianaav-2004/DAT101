...
function setSoundOnOff() { 
  ...
  menu.stopSound(); // Stops the sound when u check the box 
} 
...
function setDayNight(aEvent) { 
  isDayMode = (aEvent.target.value === "1"); // Set day or night mode based on radio buttons
  //  e.g., isDayMode = (aEvent.target.value == 1);

  // Task 2, Update the background sprite index 
...
export function resetGame() { // restart the game to countdown state, and reset score, position, and obstacles 
  hero.restart(); 
  obstacles.length = 0; 
  baits.length = 0; 
}
